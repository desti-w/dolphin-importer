...
def clickExecute(driver, wait_time, str_path):
    try:
        WebDriverWait(driver, wait_time).until(EC.presence_of_element_located((By.XPATH, str_path)))
        el_btn = driver.find_element(By.XPATH, str_path)
...
def show_test_networks(driver):
    try:
        driver.get(f'chrome-extension://{metamask_id}/home.html#settings/advanced')
...
def add_network(driver, name_network):
    ...
    try:
        ...
        driver.get(f'chrome-extension://{metamask_id}/home.html#settings/networks/add-network')
        info_panel_closer(.7)
...
def main(zero, ads_id, seed, password, unlock_mode):
    try:
        ...
        try:
            ...
            chrome_driver = str(resp["automation"]["port"])
        ...
        for q in range(3):
            clickExecute(driver, 2, XPATH_POPOVER_CLOSE)                        # Close popover windows
...
if __name__ == '__main__':
    ...
    # ===========================================Settings===============================================================
    # Metamask id
    metamask_id = 'cfkgdnlcieooajdnoehjhgbmpbiacopjflbjpnkm'
